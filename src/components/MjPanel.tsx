...
export function MjPanel() {
    ...
    const playersVisibleContacts = store.getVisibleContacts();
                ...
                <section className="flex flex-col gap-4">
                    ...
                    <div className="flex flex-col gap-4">
                        {store.contacts.map(contact => {
                            const distanceNm = getDistanceNm(store.position, contact.position);
                            ...
                            return (
                                ...
                                <div key={contact.id} className={`p-3 rounded-lg border flex flex-col gap-3 shadow-inner transition-colors ${isVisible ? "bg-slate-950 border-slate-800" : "bg-slate-900/50 border-red-900/30"}`}>
                                    ...
                                    <div className="flex justify-between items-center border-b border-slate-800/50 pb-2 gap-2">
                                        ...
                                        <button
                                            onClick={() => store.setContactToPlace(store.contactToPlace === contact.id ? null : contact.id)}
                                            className={`text-[10px] font-bold px-2 py-1 rounded whitespace-nowrap transition-colors ${store.contactToPlace === contact.id ? "bg-amber-500 text-amber-950 animate-pulse" : "bg-slate-800 text-amber-500 hover:bg-slate-700"}`}
                                        >
                                            {store.contactToPlace === contact.id ? "CIBLER..." : "📍 PLACER"}
                                        </button>
                                    ...
                                    <div className="flex justify-between items-center text-[10px]">
                                        ...
                                        <span className={`font-bold ${isVisible ? "text-cyan-500" : "text-red-500"}`}>
                                            {distanceNm.toFixed(1)} nm {isVisible ? "(Détecté par les joueurs)" : "(Caché)"}
                                        </span>
                                    </div>
